...
def backward(model, emissions):
    ...
    emissionsReverse = emissions[1:].reverse() if emissions[1:].reverse() else []
    for q in emissionsReverse:
        b_prev = util.Counter()
        for state_k in model.getStates():
            b_prev[state_k] = sum([model.a(state_k, state_l)*model.e(state_l)*b[state_l] for state_l in model.getStates()])
        b = b_prev.copy()
    for state in model.getStates():
